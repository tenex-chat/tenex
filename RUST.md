...
- **Scheduling tools**: `schedule_task` (write one-off or recurring tasks to `schedules.json` via `tenex-scheduler` storage API), `kill` (three-mode: scheduled tasks by `task-{ts}-{random}` ID via tenex-scheduler storage; shell tasks and agent executions routed to the runtime control socket — no TS state required)
- **MCP proxy tools**: loaded dynamically from `TENEX_MCP_MANIFEST` when set; each `McpProxyTool` proxies calls over `TENEX_MCP_SOCKET` (NDJSON request/response), keeping stdout free for signed Nostr events
- **Cassette recorder**: when `TENEX_LLM_CASSETTE_RECORD_PATH` is set, `CassetteRecorder` writes each LLM turn as an NDJSON record for observability/replay
- **Injection tracking**: `MessageInjectionTracker` polls the conversation DB between turns for externally injected messages; injects them as synthetic user turns before the next LLM call
- **Runtime state persistence**: `RuntimeStateHandle` persists per-turn agent state to the conversation SQLite store
...
Note: `conversation_get`, `conversation_list`, `conversation_search`, `kill`, `schedule_task`, `change_model`, `no_response`, `report_publish`, and `agents_write` are now implemented in Rust. The Rust `conversation_search` does semantic RAG search against the `"conversations"` collection (same `RagStore` used by `rag_search`); requires embedding to be configured. The Rust `kill` supports three modes: scheduled tasks by `task-{ts}-{random}` ID (tenex-scheduler storage), shell tasks, and agent executions (both via runtime control socket) — no TS in-process state required. The Rust `change_model` accepts any model spec (`provider:model`, named preset) rather than the TS restriction to meta-model variant names. The Rust `report_publish` emits kind:30023 NIP-23 articles via `PublishArticleIntent` through the standard NDJSON-stdout channel; it includes path-traversal protection and directory recursion. The Rust `agents_write` is pure file I/O against `~/.tenex/agents/<pubkey>.json` — no SQLite, no AgentProvisioningService dependency; it preserves unknown JSON fields across update so TS-written records (e.g. `category`, `eventId`, `mcpServers`) are not clobbered. The TS `send_message` is Telegram-specific (not generic Nostr) and depends on TS-only infrastructure.

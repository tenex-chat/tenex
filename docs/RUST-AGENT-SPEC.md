...
For a single file: `d_tag` = filename, `document_tag` = file stem. For a directory: `d_tag` = `dirName/relative/path`, `document_tag` = directory name. Absolute paths are unrestricted (used as-is, no containment check); relative paths are anchored to the project root and rejected if they escape it via `canonicalize() + starts_with(project_root)`. The tool is read-only on the filesystem (it reads markdown and publishes to Nostr), so unrestricted absolute paths are safe and let agents publish from `$AGENT_HOME` or other locations outside the project. Returns `{ success, published: [d_tags], summary }`. Each file emits a `PublishArticleIntent` (→ `Intent::PublishArticle`) over the standard NDJSON-stdout channel with tags `[d]`, `[document]`, `[a]` (project link).

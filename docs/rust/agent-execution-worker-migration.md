...
- The worker entrypoint supports a deterministic mock engine selected with
  `TENEX_AGENT_WORKER_ENGINE=mock`.
- The worker entrypoint also supports `TENEX_AGENT_WORKER_ENGINE=agent`, which
  boots the real `AgentExecutor` path from filesystem-backed config, metadata,
  agent storage, conversation state, and mock LLM provider config. It can also
  seed non-initial RAL continuations from the worker execute frame.
- TypeScript process-supervision tests drive both engines over the worker
  protocol and observe `execution_started`, `stream_delta`, `publish_request`,
  tool completion, delegation registration, terminal `complete`, terminal
  `waiting_for_delegation`, and terminal `no_response` messages.
- Rust process-supervision coverage can drive the mock worker lifecycle over
...
- The existing TypeScript daemon has an opt-in `TENEX_AGENT_WORKER=1` route
  ...
  fresh first-turn executions; resumptions, active RALs, delegation completions,
  kill signals, and non-initial conversations intentionally remain on the
  in-process `AgentExecutor`.
- Worker `publish_request` frames now include complete agent-signed NIP-01
  events. The TypeScript daemon bridge verifies the event hash, signature, and
  target-agent pubkey, then publishes the exact signed event without re-signing
  it. This preserves reply/delegation IDs for existing TENEX clients and proves
  the bridge contract Rust will take over.
- The Rust crate now has a filesystem-backed publish outbox at
  `publish-outbox/pending/<event-id>.json`. It accepts worker `publish_request`
  frames only after validating the worker protocol frame, expected agent pubkey,
  NIP-01 event hash, and Schnorr signature, then returns a correlated
  `publish_result.status=accepted`.
- The outbox can drain accepted pending records through a Rust publisher
  interface and durably move them to `publish-outbox/published/<event-id>.json`
  or `publish-outbox/failed/<event-id>.json` with relay attempt metadata. The
...
- Publish failure coverage verifies the bridge sends a failed `publish_result`
  frame before raising the error and clears any parent RAL that was seeded
  before a terminal worker frame arrived.

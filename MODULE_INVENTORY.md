...
| `SkillService` | `src/services/skill/` | Fetches, resolves, and renders agent skills from Nostr kind:4202 events. `fetchSkills(ids)` resolves skill event IDs into `SkillData` objects (title, content, installedFiles, eventId). `renderSkill()` (exported from `prompts/fragments/12-skills.ts`) formats skills for system prompt injection. Supports self-applied skills: agents can activate skills mid-conversation via the `skills_set` tool, which persists resolved eventIds in `ConversationStore.selfAppliedSkills` per agent pubkey. `StreamCallbacks.prepareStep` rehydrates skills from ConversationStore each step so mid-RAL changes take effect immediately. |

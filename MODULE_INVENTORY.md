...
- **Rust project-status filesystem source slice (2026-04-22)**: `crates/tenex-daemon/src/project_status_sources.rs` and `crates/tenex-daemon/src/project_status_snapshot.rs` own the filesystem readers and owned projection shape for the project-status runtime boundary that still needs main-loop wiring.

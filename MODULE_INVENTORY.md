...
- On the `rust-agent-worker-publishing` branch, the crate also includes `daemon_shell.rs`, `daemon_status.rs`, `routing_shadow.rs`, `routing_shadow_log.rs`, `worker_recovery_apply.rs`, `worker_diagnostics.rs`, `worker_dispatch_admission_start.rs`, `worker_message_flow.rs`, `worker_frame_pump.rs`, `worker_session_loop.rs`, `worker_dispatch_tick.rs`, `worker_startup_recovery.rs`, and `worker_reuse.rs`, which cover library-first daemon shell lifecycle, daemon presence snapshots, side-effect-free routing shadow records and durable shadow-decision logging, crash recovery, diagnostics snapshots, admission-to-spawn handoff, routed worker message flow, single-frame worker pump handling, bounded worker session looping, single-pass dispatch ticking, startup orphan recovery, and warm worker compatibility decisions beneath the existing supervision stack.

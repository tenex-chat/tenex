...
- **`crates/tenex-project`**: Read-side project view. Reads project metadata and membership from `projects/<dTag>/event.json`, then loads member-agent projections from global JSON via `tenex-agent-registry`. Missing member-agent files are skipped, with a read-only best-effort display-name lookup through the host `tenex-identity` socket for cleaner unavailable-agent logs. It must not mutate agent JSON, project events, or installed-agent indexes.
...
- **`crates/tenex-telegram`**: Telegram transport daemon and delivery library. Startup discovery is agent-first: `discovery.rs` scans installed agent JSON for top-level `telegram` blocks, then reads project events only to map those enabled agent pubkeys to candidate local project dTags. A bot token identifies one agent; `binding.rs` stores Telegram channel bindings as `(transport=telegram, agentPubkey, channelId) -> projectId` under `data/transport-bindings.json`, so DMs, whole groups, and group topics can route the same agent to different projects. It must not call `Project::agents()` during host startup discovery because missing project members are a normal stale/remote-project condition, not a Telegram discovery problem.
- **`crates/tenex-agent`**: Rust agent runner and tools. Agent-management tools that write installed-agent files delegate to `tenex-agent-registry` instead of writing JSON directly. Runtime-provided MCP tools are exposed as proxy `ToolDyn` instances from the manifest/socket passed by `tenex`; ACP-backed agents also get runner-owned TENEX coordination tools through the local `tenex` MCP stdio wrapper. Runner-owned prompt inputs such as root `AGENTS.md` are loaded here before calling `tenex-system-prompt`.
- **`tenex`**: Rust CLI/runtime supervisor. `runtime_cmd` owns relay subscriptions, dispatch coordination, kind:24020 agent config update ingestion, control socket handling, per-run MCP bridge setup, ACP/TENEX runner selection, and the live read-side snapshot of project agents. It watches `<base_dir>/agents/*.json` and reloads project-agent projections, subscriptions, project status, and queued dispatch jobs when installed-agent configuration changes.
...
- **Presentation**: `presenters/ConversationPresenter` transforms catalog data for display with short conversation IDs; full-ID lookup is handled by the centralized prefix store. The presentation layer sits between `ConversationCatalogService` (data) and tools/UI (consumers).
...
| `SkillService` | `src/services/skill/` | `SkillService` resolves the effective local skill set (see below) and fetches active skill content for prompt injection. Skills live exclusively on disk; no Nostr-event hydration path. `tool-permissions.ts` exposes helpers for the only-tools / allow-tools / deny-tools directives declared in SKILL.md frontmatter. |
...
| `RAG*` services | `src/services/rag/` | Manage vector-store collections, document ingestion, query APIs, subscriptions, and embedding integration (`EmbeddingProviderFactory`, `RagSubscriptionService`). Tools in `src/tools/implementations/rag_*` should call these. |
| `UnifiedSearchService` + `ContextDiscoveryService` | `src/services/search/` | Coordinate cross-collection search over conversations, lessons, and scoped RAG collections. `ContextDiscoveryService` runs the proactive, latency-bounded context discovery pass for new conversations or configured triggers, emitting pointer-only reminder hints plus retrieval usage telemetry while delegating actual retrieval to existing search providers and tools. |
...
| `SkillService` | `src/services/skill/` | `SkillService` resolves the effective local skill set across `$TENEX_BASE_DIR/home/<agent-short-pubkey>/skills/<id>/`, `$TENEX_BASE_DIR/projects/<project-dTag>/skills/<id>/`, `$TENEX_BASE_DIR/skills/<id>/`, and `~/.agents/skills/<id>/` with precedence `agent > project > global > ~/.agents`. Skills are installed on disk by external tooling; metadata comes solely from `SKILL.md` YAML frontmatter. Agents refer to skills by local directory ID — `SkillData.identifier` is always that local ID. `skills_set` validates against the effective local skill set and persists those IDs in `ConversationStore.selfAppliedSkills` per agent pubkey. `renderSkill()` (in `agents/execution/skill-reminder-renderers.ts`) formats skills for system prompt injection, and `StreamCallbacks.prepareStep` rehydrates stored local skill IDs each step so mid-RAL changes take effect immediately. |
...
- **Services Reorganization (COMPLETED 2025-12-20)**: Grouped related services into subdirectories: `projects/` (ProjectContext, ProjectContextStore), `embedding/` (EmbeddingProvider), `scheduling/` (SchedulerService), `skill/` (SkillService), `agents/` (NDKAgentDiscovery). Each subdirectory has an `index.ts` barrel export.

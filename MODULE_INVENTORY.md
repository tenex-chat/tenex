...
- **`crates/tenex-daemon/src/backend_events_tick.rs`**: Central backend-events periodic tick runner over `periodic_tick`. It registers the backend-status task and per-project status tasks on the same in-memory scheduler, takes due work once, and enqueues backend heartbeat, installed-agent-list, and project-status records through their filesystem-backed runtimes.

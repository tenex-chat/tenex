...
| `ChannelSessionStore` | `src/services/ingress/ChannelSessionStore.ts` | Persists per-channel conversation continuity for non-Nostr gateways. Telegram DMs/groups use it to map `(project, agent, channel)` to the last inbound message ID and active conversation ID so later turns can be bridged back into the correct conversation without native Nostr threading. |
...
| `Telegram*` services | `src/services/telegram/` | Telegram transport gateway and delivery adapters. `TelegramGatewayCoordinator` is the daemon-facing shared poller for bot tokens reused across multiple project runtimes; it persists first-contact project bindings via `TelegramChannelBindingStore` / `TelegramPendingBindingStore`, prompts users to choose a project when a DM/group is ambiguous, normalizes updates through `TelegramInboundAdapter`, and uses `TelegramDeliveryService` / `TelegramRuntimePublisher` to deliver replies back to Telegram while preserving Nostr as the canonical agent identity substrate. `TelegramGatewayService` remains the single-project gateway used by focused tests and lower-level harnesses. |

...
- **`src/event-handler`**: Domain orchestrators triggered by incoming Nostr events. `reply` now terminates at the transport-neutral ingress seam by normalizing Nostr events through `nostr/NostrInboundAdapter` and delegating the canonical envelope to `services/ingress/RuntimeIngressService`. `kill-signal.ts` handles kind:24136 (TenexKillSignal) control-plane events: it converts the Nostr event into a canonical envelope with `isKillSignal: true` and routes it through `RuntimeIngressService` so `DelegationCompletionHandler`'s implicit-kill branch can wake the immediate parent RAL without appending anything to conversation stores. Kill signals are also dispatched in-process by `AgentDispatchService.dispatchKillWakeup()` (called from `kill.ts` after local abort state is committed), bypassing the Nostr relay round-trip for same-process kills.

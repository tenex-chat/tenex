...
- **`crates/tenex-daemon`**: Future Rust control-plane crate for the daemon migration described in `docs/rust/`. Its current executable responsibility is cross-language compatibility verification and library-first migration slices: shared fixture compatibility, filesystem daemon state, subscription/routing decisions, worker protocol frames, RAL journal/scheduler state, dispatch queues, publish outbox maintenance/runtime boundaries, relay publishing, backend event encoders/signer/config readers, operations-status runtime projection, filesystem caches, scheduler wakeups, worker process supervision, warm-reuse compatibility planning, and worker runtime orchestration boundaries. The crate must not take production daemon authority until the milestone gates in `docs/rust/implementation-milestones-and-quality-gates.md` are satisfied.
...
- **Core Runtime**: `Daemon`, `ProjectRuntime`, and `SubscriptionManager` run background loops that subscribe to relays and process events. `ProjectRuntime` also persists the shared project descriptor at `$TENEX_BASE_DIR/projects/<project-dTag>/project.json` on start/stop so Rust can discover active project-status inputs without live TypeScript object access.
...
- **Rust project-status descriptor discovery slice (2026-04-22)**: `crates/tenex-daemon/src/project_status_descriptors.rs` reads active project descriptors from `$TENEX_BASE_DIR/projects/<project-dTag>/project.json`, while `src/daemon/ProjectRuntime.ts` writes the same descriptor on project start and marks it stopped during teardown. This makes project-status tick inputs filesystem-based instead of requiring live TypeScript context.

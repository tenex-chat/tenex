...
- **`src/events`**: Typed schemas, utils, and constants for every event TENEX produces or consumes (Nostr kinds, internal telemetry events). Also hosts layer-2 runtime contracts and testing adapters that higher layers can depend on without importing concrete transport implementations (for example `events/runtime/AgentRuntimePublisher.ts`, `AgentRuntimePublisherFactory.ts`, `InboundEnvelope.ts`, `LocalInboundAdapter.ts`, `RecordingRuntimePublisher.ts`). The delegation kill-signal wake-up path now depends on `InboundEnvelope` preserving aborted `DelegationMarker` carrier fields (`delegationConversationId`, `parentConversationId`, `recipientPubkey`, `completedAt`, `abortReason`) across runtime publishers and event-handler ingress. Treat as a contract; modifications require updates to this file.
- **`src/event-handler`**: Domain orchestrators triggered by incoming Nostr events. `reply` now terminates at the transport-neutral ingress seam by normalizing Nostr events through `nostr/NostrInboundAdapter` and delegating the canonical envelope to `services/ingress/RuntimeIngressService`. `kill-signal.ts` is the control-plane wake-up adapter for aborted `DelegationMarker` traffic: it forwards only aborted kill markers into ingress so `AgentDispatchService` can resume the immediate parent without appending the signal to the killed child transcript.

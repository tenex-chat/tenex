...
- On the `rust-agent-worker-publishing` branch, the crate also includes `daemon_status.rs`, `routing_shadow.rs`, `worker_recovery_apply.rs`, `worker_diagnostics.rs`, `worker_dispatch_admission_start.rs`, `worker_message_flow.rs`, `worker_dispatch_tick.rs`, `worker_startup_recovery.rs`, and `worker_reuse.rs`, which cover daemon presence snapshots, side-effect-free routing shadow records, crash recovery, diagnostics snapshots, admission-to-spawn handoff, routed worker message flow, single-pass dispatch ticking, startup orphan recovery, and warm worker compatibility decisions beneath the existing supervision stack.

...
TENEX is a Bun CLI with its runtime entry in `src/tenex.ts`; core domains live in `src/agents`, `src/commands`, `src/conversations`, `src/nostr`, and `src/tools`, all accessed with the `@/` alias. Scripts covering builds, telemetry, and compatibility reside in `scripts/`, automation templates are in `tools/`, bundled output lands in `dist/`, and e2e docs live in `tests/` and `E2E_TESTING_ARCHITECTURE.md`.
...
`bun run start` exercises the CLI entry, while `bun run build` produces the bundled distribution in `dist/`. Run `bun test` before every push; add `--watch` or `--coverage` locally when needed. `bun run typecheck` (wrapper around `scripts/typecheck.sh`) enforces strict TS flags, `bun run lint` applies ESLint, and `bun run validate:events` plus `bun run scripts/test-ios-compat.sh` cover protocol-specific validations.

## Coding Style & Naming Conventions
Source files are TypeScript ES modules formatted by Biome (4 spaces, double quotes, trailing commas) and linted by ESLint; keep strict compiler flags (`strict`, `noUnused*`, `isolatedModules`) satisfied. Directories and files use kebab-case, classes use PascalCase, and functions/constants use camelCase. Prefer domain folders over dumping helpers in `utils/`, and import shared helpers via `@/…` so bundling stays consistent.

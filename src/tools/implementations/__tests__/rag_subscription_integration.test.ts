...
describe('RAG Subscription Integration Tests', () => {
  ...
  describe('Resource Update Data Pipeline', () => {
    ...
    test('should handle various notification formats correctly', async () => {
      ...
      expect(addDocumentsSpy.mock.calls[1][1][0].content).toBe('{"key":"value","nested":{"data":"here"}}');
      expect(addDocumentsSpy.mock.calls[2][1][0].content).toBe('Text format');
      expect(addDocumentsSpy.mock.calls[3][1][0].content).toBe('{"someField":"fallback format"}');
  ...
  describe('Statistics Optimization', () => {
    test('list tool should calculate statistics in single pass', async () => {
      ...
      await service.createSubscription('sub2', 'test-agent-pubkey-valid', 'server2', 'res2', 'test-collection', 'Sub 2');
      await service.createSubscription('sub3', 'test-agent-pubkey-valid', 'server3', 'res3', 'test-collection', 'Sub 3');

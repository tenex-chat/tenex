...
const codebaseSearchSchema = z.object({
  ...
  includeSnippets: z.boolean().nullable()
    .default(false)
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (!ignoreDirs.includes(entry.name)) {
          ...
          await recursiveFileSearch(fullPath, query, fileType, results, maxResults, entryPath);
        }
  ...
  try {
    ...
    if (stdout) {
      ...
      for (const match of matches) {
        ...
        const lineNumber = parseInt(afterPath.substring(0, secondColonIndex));

...
class TestableEventMonitor extends EventMonitor {
  public async testHandleEvent(event: NDKEvent): Promise<void> {
    ...
    return (this as unknown as { handleEvent: (event: NDKEvent) => Promise<void> }).handleEvent(
      event
    );
  }
}
...
describe("EventMonitor", () => {
  ...
  describe("handleEvent", () => {
    const createMockEvent = (tags: string[][], pubkey = "testpubkey"): NDKEvent => {
      return {
        id: "event123",
        kind: 1,
        tags,
    ...
    it("should handle errors in event processing", async () => {
      mockProcessManager.isProjectRunning = mock(() => Promise.reject(new Error("Check failed")));
      const event = createMockEvent([["a", "30311:pubkey123:myproject"]]);

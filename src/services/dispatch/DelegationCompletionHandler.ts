...
export async function handleDelegationCompletion(
    ...
    try {
        ...
        span.setAttribute("delegation.event_id", resolvedDelegationEventId || "unknown");
        ...
        if (!resolvedDelegationEventId) {
            ...
            span.setStatus({ code: SpanStatusCode.ERROR, message: "Missing delegation event id" });
            logger.warn(
                "[handleDelegationCompletion] Found waiting delegation location without delegation event id",
                {
                    agentPubkey: location.agentPubkey.substring(0, 8),

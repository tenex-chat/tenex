...
describe("RAL Delegation Flow", () => {
    ...
    describe("Implicit-kill branch (kill-signal envelopes)", () => {
        const PARENT_CONV_ID = "parent-conv-kill-test-00";
        const CHILD_CONV_ID = "child-conv-kill-test-11";
        ...
        it("resolves the parent wake target from a completed/aborted delegation", async () => {
            // Setup: parent agent has a pending delegation to child
            const ralNumber = registry.create(mockAgent.pubkey, PARENT_CONV_ID, PROJECT_ID);
            registry.mergePendingDelegations(mockAgent.pubkey, PARENT_CONV_ID, ralNumber, [
                {
                    delegationConversationId: CHILD_CONV_ID,
                    recipientPubkey: mockAgent2.pubkey,
                    senderPubkey: mockAgent.pubkey,
                    prompt: "do something",
                    ralNumber,
            ...
            // State commit: move delegation to completed/aborted as markParentDelegationKilled does
            registry.markParentDelegationKilled(CHILD_CONV_ID);

            // Receive kill-signal envelope
            const envelope = buildKillSignalEnvelope(CHILD_CONV_ID);
            const result = await handleDelegationCompletion(envelope);

            expect(result.recorded).toBe(true);
            expect(result.agentSlug).toBe(mockAgent.slug);
            expect(result.conversationId).toBe(PARENT_CONV_ID);
            // Control-plane: kill signal must NOT have been added to child conv store
            expect(conversationStoreAddEnvelopeSpy).not.toHaveBeenCalledWith(CHILD_CONV_ID, envelope);
        });

        it("returns { recorded: false } for a duplicate kill signal (idempotent no-op)", async () => {
            const ralNumber = registry.create(mockAgent.pubkey, PARENT_CONV_ID, PROJECT_ID);
            registry.mergePendingDelegations(mockAgent.pubkey, PARENT_CONV_ID, ralNumber, [
                {
                    delegationConversationId: CHILD_CONV_ID,
            ...
            // First signal consumes the wake target
            const envelope = buildKillSignalEnvelope(CHILD_CONV_ID);
            const first = await handleDelegationCompletion(envelope);
            expect(first.recorded).toBe(true);

            // markParentDelegationKilled moves the entry to completed/aborted.
            // After the parent resumes and clearCompletedDelegations runs, the
            // delegation is gone, so a second signal finds nothing.
            registry.clearCompletedDelegations(mockAgent.pubkey, PARENT_CONV_ID, ralNumber);

            const second = await handleDelegationCompletion(buildKillSignalEnvelope(CHILD_CONV_ID));
            expect(second.recorded).toBe(false);
        });

        it("returns { recorded: false } when the delegation is still pending (not yet committed)", async () => {
            const ralNumber = registry.create(mockAgent.pubkey, PARENT_CONV_ID, PROJECT_ID);
            registry.mergePendingDelegations(mockAgent.pubkey, PARENT_CONV_ID, ralNumber, [
                {
                    delegationConversationId: CHILD_CONV_ID,
            ...
            // Do NOT call markParentDelegationKilled — delegation remains pending
        ...
        it("does not append the kill-signal envelope to the child conversation store", async () => {
            const ralNumber = registry.create(mockAgent.pubkey, PARENT_CONV_ID, PROJECT_ID);
            registry.mergePendingDelegations(mockAgent.pubkey, PARENT_CONV_ID, ralNumber, [
                {
                    delegationConversationId: CHILD_CONV_ID,
            ...
            // The child conversation store must not have received the control-plane signal
            expect(conversationStoreAddEnvelopeSpy).not.toHaveBeenCalledWith(CHILD_CONV_ID, expect.anything());
        });
    });
});

    ...
    describe("status event publishing", () => {
        it("should publish properly signed status events", async () => {
            await withTestEnvironment(async (fixture) => {
                ...
                const { user: projectOwner, signer } = await getTestUserWithSigner(
                    "alice",
                    fixture.ndk
                );
                ...
                expect(eventData.tags).toContainEqual(expect.arrayContaining(["status", "online"]));
            });
        });
                ...
                mock.module("@/services", () => ({
                    getProjectContext: () => ({
                        project: {
                            ...
                            tagReference: () => [
                                "a",
                                `31933:${projectOwner.pubkey}:detailed-project`,
                            ],
                        },
                        signer,
                        agents: new Map([
                            [
                                "analyzer",
                                { pubkey: agent1.pubkey, name: "Analyzer", slug: "analyzer" },
                            ],
                            [
                                "validator",
                                { pubkey: agent2.pubkey, name: "Validator", slug: "validator" },
                            ],
                        ]),
                    }),
                    isProjectContextInitialized: () => true,

...
import { RAGOperations, RAGValidationError, RAGOperationError } from '../RAGOperations';
...
describe('RAGOperations', () => {
    ...
    describe('createCollection', () => {
        ...
        it('should throw error for invalid collection names', async () => {
            ...
            for (const name of invalidNames) {
                await expect(operations.createCollection(name))
                    .rejects
        ...
        it('should throw error if collection already exists', async () => {
            ...
            await expect(operations.createCollection('existing_collection'))
                .rejects
                .toThrow(RAGOperationError);
        });
    });
    ...
    describe('addDocuments', () => {
        ...
        it('should throw error for empty documents array', async () => {
            await expect(operations.addDocuments('test_collection', []))
                .rejects
                .toThrow(RAGValidationError);
        });
        ...
        it('should throw error for documents with empty content', async () => {
            ...
            for (const doc of invalidDocs) {
                await expect(operations.addDocuments('test_collection', [doc]))
                    .rejects
                    .toThrow(RAGValidationError);
            }
        });
        ...
        it('should handle batch processing for large document sets', async () => {
            const largeDocSet = Array(250).fill(null).map((_, i) => ({
                content: `Document ${i}`
            }));
    ...
    describe('performSemanticSearch', () => {
        ...
        it('should validate empty query text', async () => {
            await expect(operations.performSemanticSearch('test_collection', '', 5))
                .rejects
                .toThrow(RAGValidationError);
        ...
        it('should validate topK parameter', async () => {
            const invalidTopKs = [0, -1, 101, 0.5, NaN];
        ...
        it('should handle concurrent queries', async () => {
            ...
            const queries = Array(10).fill(null).map((_, i) => 
                operations.performSemanticSearch(
                    'test_collection',
                    `query ${i}`,
                    5
                )
    ...
    describe('deleteCollection', () => {
        ...
        it('should throw error if collection does not exist', async () => {
            ...
            await expect(operations.deleteCollection('non_existent'))
                .rejects
                .toThrow(RAGOperationError);
        });
    });
    ...
    describe('Edge Cases', () => {
        it('should handle dimension mismatch', async () => {
            mockDbManager.tableExists = jest.fn().mockResolvedValue(false);
            mockDbManager.createTable = jest.fn().mockRejectedValue(
                new Error('Vector dimension mismatch')
            );

            await expect(operations.createCollection('test_collection'))
                .rejects
                .toThrow(RAGOperationError);
        });

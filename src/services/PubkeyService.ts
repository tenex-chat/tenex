...
export class PubkeyService {
    ...
    getNameSync(pubkey: Hexpubkey): string {
        ...
        // Return shortened pubkey as fallback using shared DISPLAY_PREFIX_LENGTH constant
        return pubkey.substring(0, DISPLAY_PREFIX_LENGTH);
    }
    ...
    private extractDisplayName(profile: UserProfile, pubkeyFallback?: string): string {
        ...
        if (pubkeyFallback) {
            return pubkeyFallback.substring(0, DISPLAY_PREFIX_LENGTH);
        }
                                ...
                                batch.map(async (pubkey) => {
                                    try {
                                        ...
                                        logger.warn("[PUBKEY_SERVICE] Failed to warm profile", {
                                            pubkey: pubkey.substring(0, DISPLAY_PREFIX_LENGTH),
                                            error: error instanceof Error ? error.message : String(error),
                                        });
                                        results.set(pubkey, pubkey.substring(0, DISPLAY_PREFIX_LENGTH));
                                    }
                                })
                            );
                        } finally {

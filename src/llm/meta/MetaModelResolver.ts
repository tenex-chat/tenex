 ...
 */
function stripKeywordsFromMessage(
    message: string,
    matchedKeywords: string[]
): string {
    let result = message;
...
export function resolve(
    ...
    // Build keyword map
    const keywordMap = buildKeywordMap(config);
    ...
    const matches = findMatchingKeywords(message, keywordMap);
    ...
    const winner = selectWinningVariant(matches);
    if (!winner) {
 ...
 */
export function resolveToVariant(
    config: MetaModelConfiguration,
    variantName: string
): MetaModelResolution {
 ...
 */
export function isMetaModel(config: unknown): config is MetaModelConfiguration {

 ...
 */
export async function selectOllamaModel(currentModel?: string, baseUrl?: string): Promise<string> {
    ...
    if (result.status === "not_found") {
        ...
        return await promptManualModel(currentModel || "");
    }
 ...
 */
export async function selectOpenRouterModel(currentModel?: string): Promise<string> {
    ...
    if (openRouterModels.length > 0) {
        ...
        if (selectedModel === "__manual__") {
            return await promptManualModel(currentModel || "openai/gpt-4");
        }
 ...
 */
export async function selectModelsDevModel(
    provider: string,
    defaultModel?: string
    ...
    if (models.length > 0) {
        ...
        if (selectedModel === "__manual__") {
            const id = await promptManualModel(defaultModel || "");
            return { id, name: id };
        }

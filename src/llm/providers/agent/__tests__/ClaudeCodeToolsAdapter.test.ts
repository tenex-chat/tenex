...
describe("ClaudeCodeToolsAdapter", () => {
    ...
    const createMockTenexTool = (description = "Test tool"): CoreTool => ({
        description,
    ...
    const createMockExternalMcpTool = (description = "External MCP tool"): CoreTool => ({
        description,
    ...
    describe("createSdkMcpServer", () => {
        it("should filter out external MCP tools", () => {
            ...
            ClaudeCodeToolsAdapter.createSdkMcpServer(tools, "tenex");
        ...
        it("should pass web_fetch and web_search tools through (not filtered)", () => {
            // web_fetch/web_search are no longer filtered — TENEX provides these via the
            // MCP server, and Claude's WebFetch/WebSearch built-ins are disabled via disallowedTools
            const tools: Record<string, CoreTool> = {
        ...
        it("should pass fs_* and shell tools through (not filtered)", () => {
            // fs_* and shell tools pass through the adapter — TENEX controls access
            // Claude Code's built-in equivalents are disabled via disallowedTools
            const tools: Record<string, CoreTool> = {
        ...
        it("should return undefined when all tools are filtered out (only external MCP)", () => {
            const tools: Record<string, CoreTool> = {
                "mcp__external__tool": createMockExternalMcpTool(),
            };

            const server = ClaudeCodeToolsAdapter.createSdkMcpServer(tools, "tenex");
        ...
        it("should use the provided serverName", () => {
            const tools: Record<string, CoreTool> = {
                delegate: createMockTenexTool(),
            };

            ClaudeCodeToolsAdapter.createSdkMcpServer(tools, "tenex_2");

            expect(createSdkMcpServerCalls).toHaveLength(1);
            expect(createSdkMcpServerCalls[0].name).toBe("tenex_2");
        });
        ...
        it("should properly handle mix of all tool types", () => {
            const tools: Record<string, CoreTool> = {
                ...
                todo_write: createMockTenexTool(),
                // fs_* and shell tools pass through (NOT filtered)
                fs_read: createMockTenexTool(),
                fs_write: createMockTenexTool(),
                shell: createMockTenexTool(),
                // web tools pass through (NOT filtered)
                web_fetch: createMockTenexTool(),
                web_search: createMockTenexTool(),
            ...
            expect(createSdkMcpServerCalls).toHaveLength(1);
            const callArgs = createSdkMcpServerCalls[0];
            // All tools except external MCP tools: 11 TENEX tools pass through
            ...
            expect(toolNames).toContain("web_fetch");
            expect(toolNames).toContain("web_search");
            // Should be filtered (external MCP tools only)
        ...
        it("should handle primitive schema types without throwing", () => {
            ...
            const toolWithPrimitiveSchema: CoreTool = {
            ...
            // Should not throw
            expect(() => ClaudeCodeToolsAdapter.createSdkMcpServer(tools, "tenex")).not.toThrow();

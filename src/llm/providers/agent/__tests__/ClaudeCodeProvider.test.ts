...
describe("ClaudeCodeProvider", () => {
    ...
    describe("createAgentSettings integration", () => {
        ...
        it("should use unique MCP server name when external server is named 'tenex'", () => {
            ...
            // Provide an external MCP server named "tenex" — internal server should get a different name
            const model = provider.createModel("claude-sonnet-4-20250514", {
                tools,
                workingDirectory: "/test/path",
                mcpConfig: {
            ...
            const settings = model.agentSettings as { mcpServers?: Record<string, unknown> };
            expect(settings.mcpServers).toBeDefined();
            // Both the internal TENEX server and external "tenex" server should be present
            // The internal server name should be unique (e.g. "tenex_2")
            const serverNames = Object.keys(settings.mcpServers ?? {});
            expect(serverNames).toContain("tenex"); // external server
            // Internal server gets a non-colliding name
            const internalServer = serverNames.find((name) => name !== "tenex");

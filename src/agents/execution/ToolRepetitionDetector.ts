...
export class ToolRepetitionDetector {
  ...
  constructor(maxHistory = 20, repetitionThreshold = 3) {
  ...
  private generateWarningMessage(tool: string, count: number): string {
    return `⚠️ SYSTEM: You have called the '${tool}' tool ${count} times with identical parameters. The tool is working correctly and returning results. Please process the tool output and continue with your task, or try a different approach. Do not call this tool again with the same parameters.`;
  }

...
describe("ThreadWithMemoryStrategy - Sub-thread Bug Reproduction", () => {
    ...
    describe("ParticipationIndex behavior", () => {
        it("should track all agent participations including sub-threads", () => {
            ...
            const agentParticipations = participationIndex.getAgentParticipations(
                "test-conv",
                agentPubkey
            );

            console.log(
                "Agent participation event IDs:",
                agentParticipations.map((id) => id.substring(0, 8))
            );
        ...
        it("should identify unique thread roots where agent participated", () => {
            ...
            console.log(
                "Thread roots where agent participated:",
                threadRoots.map((id) => id.substring(0, 8))
            );
    ...
    describe("The Core Problem", () => {
        it("should demonstrate that sub-threads rooted at agent responses are missed", () => {
            ...
            console.log(
                "Current approach finds thread roots:",
                Array.from(threadRoots).map((id) => id.substring(0, 8))
            );
            ...
            const agentResponsesWithReplies = events.filter(
                (e) =>
                    e.pubkey === agentPubkey && events.some((other) => other.tagValue("e") === e.id)
            );

            console.log(
                "Agent responses that have replies (sub-thread roots):",
                agentResponsesWithReplies.map((e) => ({

...
describe("FlattenedChronologicalStrategy - Mock Scenarios", () => {
    ...
    describe("Complex Threading Scenario", () => {
        ...
        it("Bob should see Alice's task assignment and Charlie's review but not Diana's separate branch", async () => {
            ...
            // Bob should see:
            // 1. Root announcement
            expect(messageContents.some(c => c.includes("Starting new feature: Dark Mode"))).toBe(true);

            // 2. Alice's task assignment (his triggering event)
            expect(messageContents.some(c => c.includes("please implement the dark mode toggle"))).toBe(true);

            // 3. Diana's testing (root-level sibling)
            expect(messageContents.some(c => c.includes("I'll start testing the dark mode"))).toBe(true);
            ...
            expect(messageContents.some(c => c.includes("Charts don't update colors"))).toBe(false);
        });
        ...
        it("Charlie should see Bob's implementation in his thread path", async () => {
            ...
            expect(messageContents.some(c => c.includes("Starting new feature: Dark Mode"))).toBe(true);
            expect(messageContents.some(c => c.includes("please implement the dark mode toggle"))).toBe(true);
            expect(messageContents.some(c => c.includes("I'll add the toggle to the settings panel"))).toBe(true);

            // Charlie should NOT see root-level siblings (he's at depth 4)
            expect(messageContents.some(c => c.includes("I'll start testing the dark mode"))).toBe(false);
    ...
    describe("Root Collaboration Scenario", () => {
        ...
        it("All agents at root level should see each other's contributions", async () => {
            ...
            expect(messageContents.some(c => c.includes("optimizing our database queries"))).toBe(true);
            ...
            expect(messageContents.some(c => c.includes("benchmark the before/after"))).toBe(true);
    ...
    describe("Delegation Chain Scenario", () => {
        ...
        it("Bob should see Alice's delegation request and Diana's sub-delegation", async () => {
            ...
            // Bob should see:
            // 1. Root request
            expect(messageContents.some(c => c.includes("user authentication with OAuth"))).toBe(true);
        ...
        it("Diana should see the full delegation chain when responding", async () => {
            ...
            expect(messageContents.some(c => c.includes("user authentication with OAuth"))).toBe(true);
            expect(messageContents.some(c => c.includes("can you implement OAuth"))).toBe(true);
            expect(messageContents.some(c => c.includes("Starting OAuth implementation"))).toBe(true);
            expect(messageContents.some(c => c.includes("can you test the OAuth flow"))).toBe(true);
        });
    });
});
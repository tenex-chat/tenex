...
describe('ThreadWithMemoryStrategy - Fix Verification', () => {
  ...
  it('should include sub-thread context when agent has participated in sub-threads', async () => {
    ...
    messages.forEach((msg, i) => {
      if (msg.content && msg.content.length > 100) {
        console.log(`[${i}] Role: ${msg.role}, Content: ${msg.content.substring(0, 100)}...`);
      } else {
    ...
    const hasSubThreadContext = messageContents.some(c => c === '1.1') ||
                                messageContents.some(c => c?.includes('1.1'));
    const hasSubThreadRequest = messageContents.some(c => c === 'say "1.1"') ||
  ...
  it('should properly organize threads with sub-thread marked separately', async () => {
    ...
    const hasPreviousThreadMarker = systemContent.includes('Previous thread') ||
                                    systemContent.includes('previous participations');
    const hasCurrentThreadMarker = systemContent.includes('Current thread') ||
                                   systemContent.includes('current thread');
  ...
  it('should correctly identify agent responses as sub-thread roots', async () => {
    ...
    const agentEventsWithReplies = conversation.history.filter(e =>
    ...
    agentEventsWithReplies.forEach(e => {
      ...
      console.log(`Agent event "${e.content}" (${e.id.substring(0, 8)}) has ${replies.length} replies:`);
      replies.forEach(r => {
    ...
    const repliesTo1 = conversation.history.filter(e => e.tagValue('e') === agentEventsWithReplies[0].id);

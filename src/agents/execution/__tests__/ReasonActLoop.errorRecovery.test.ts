...
describe("ReasonActLoop - Error Recovery", () => {
    ...
    it("should validate tool result format", async () => {
        ...
        ).rejects.toThrow("Tool 'analyze' returned invalid result format. Missing or invalid __typedResult.");
    });
    ...
    it("should handle error event in stream", () => {
        ...
        const handleErrorEvent = (reasonActLoop as any).handleErrorEvent.bind(
            reasonActLoop
        );
        ...
        const nonOrchestratorContext = {
            agent: { name: "executor", isOrchestrator: false },
        };

        handleErrorEvent(event, state, streamPublisher, tracingLogger, nonOrchestratorContext);

        expect(tracingLogger.error).toHaveBeenCalledWith("Stream error", { error: "Connection timeout" });
        ...
        const orchestratorContext = {
            agent: { name: "orchestrator", isOrchestrator: true },
        };

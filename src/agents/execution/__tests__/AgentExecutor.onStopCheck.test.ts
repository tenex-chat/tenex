...
describe("AgentExecutor.onStopCheck - Tool Message Preservation", () => {
    ...
    describe("Integration with RALRegistry.saveState", () => {
        it("should correctly save tool messages to RAL state", () => {
            const ralNumber = ralRegistry.create(AGENT_PUBKEY, CONVERSATION_ID, "trigger-123");

            const delegateInput = {
                delegations: [{ recipient: "researcher", prompt: "Research this" }],
            };
            ...
            const delegateOutput = {
                ...
                pendingDelegations: [{
                    eventId: "del-ev-123",
                    recipientPubkey: "researcher-pk",
                    recipientSlug: "researcher",
                    prompt: "Research this",
                }],
                delegationEventIds: { "researcher-pk": "del-ev-123" },
            ...
            // Simulate the base messages that would exist
            const baseMessages: ModelMessage[] = [
                { role: "system", content: "You are a helpful assistant." },
                { role: "user", content: [{ type: "text", text: "Please delegate to researcher" }] },
            ];

            // Run the mapping
            const messagesWithTools = simulateOnStopCheckMapping(stepResult, baseMessages);

            // Save to RAL (as AgentExecutor does)
            ...
            expect(savedMessages).toHaveLength(4); // system + user + assistant + tool

            // Verify tool call was preserved
            const savedAssistant = savedMessages.find(m => m.role === "assistant");
            const savedToolCall = (savedAssistant!.content as any[])[0];
            expect(savedToolCall.input).toEqual(delegateInput);
            expect(savedToolCall.input.delegations[0].recipient).toBe("researcher");

            // Verify tool result was preserved
            const savedTool = savedMessages.find(m => m.role === "tool");
            const savedToolResult = (savedTool!.content as any[])[0];
            expect(savedToolResult.output).toEqual(delegateOutput);
            expect(savedToolResult.output.__stopExecution).toBe(true);
            expect(savedToolResult.output.message).toBe("Delegated to researcher");
        });
    });

...
function displayQueueStatus(status: unknown, detailed?: boolean): void {
  ...
  if (typedStatus.lock) {
    ...
    if (detailed) {
      ...
      console.log(
        chalk.gray(
          `  Started: ${startTime.toLocaleString()} (${formatDistanceToNow(startTime, { addSuffix: true })})`
        )
      );
      ...
      console.log(
        chalk.gray(
          `  Timeout: ${timeoutTime.toLocaleString()} (${formatDistanceToNow(timeoutTime, { addSuffix: true })})`
        )
      );
    }
  } else {
  ...
  if (queueLength > 0) {
    ...
    if (detailed) {
      ...
      for (let i = 0; i < displayCount; i++) {
        ...
        console.log(
          chalk.gray(`     Queued: ${formatDistanceToNow(queueTime, { addSuffix: true })}`)
        );
  ...
  if (
    detailed &&
    statusWithTimeouts.activeTimeouts &&
    statusWithTimeouts.activeTimeouts.length > 0
  ) {
    console.log();

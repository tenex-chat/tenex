...
export const agentRemoveCommand = new Command("remove")
  ...
  .option("--project <dTag>", "Project d-tag to remove agent from (if not specified, removes globally)")
    ...
    try {
      ...
      if (options.project) {
        ...
        if (!agent.projects.includes(options.project)) {
          logger.error(`Agent "${slug}" is not associated with project ${options.project}`);
          process.exit(1);
        }
        ...
        if (otherProjects.length > 0) {
          ...
          logger.info(`✅ Agent "${agent.name}" completely removed (was only in that project)`);
        }
      } else {
        // Remove globally
        if (!options.force) {

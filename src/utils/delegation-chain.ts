...
export function buildDelegationChain(
    ...
    const resolveDisplayName = (pubkey: string): { displayName: string; isUser: boolean } => {
        ...
        return { displayName: pubkey.substring(0, DISPLAY_PREFIX_LENGTH), isUser: false };
    };
...
export function formatDelegationChain(
    ...
    for (let i = 0; i < chain.length - 1; i++) {
        ...
        // Truncate to DISPLAY_PREFIX_LENGTH chars for display (full IDs are stored in chain entries)

    ...
    describe("with real NDK events and signatures", () => {
        it("should create properly signed completion events", async () => {
            await withTestEnvironment(async (fixture) => {
                // Setup agent
                const { user: agentUser, signer } = await getTestUserWithSigner(
                    "alice",
                    fixture.ndk
                );
        ...
        it("should handle multi-agent delegation chains", async () => {
            await withTestEnvironment(async (fixture) => {
                // Setup agents
                const { user: coordinator, signer: coordinatorSigner } =
                    await getTestUserWithSigner("alice", fixture.ndk);
                ...
                userRequest.tags.push(["d", "task-001"], ["priority", "high"]);
                ...
                expect(delegationEvent.content).toContain(
                    "Extract key points from document and create visual summary"
                );
        ...
        it("should handle conversation flow with proper threading", async () => {
            await withTestEnvironment(async (fixture) => {
                ...
                expect(status.tags).toContainEqual(expect.arrayContaining(["e", initial.id]));

                // Result should reference status
                expect(result.tags).toContainEqual(expect.arrayContaining(["e", status.id]));

                // Thanks should reference result
                expect(thanks.tags).toContainEqual(expect.arrayContaining(["e", result.id]));

...
/// Result of `spawn_daemon`. The original CLI process gets `Caller`; the
/// fully-detached grandchild gets `Daemon`. The intermediate child does its
/// second fork and exits internally — callers never observe it.
pub enum Role {
    Caller,
    Daemon,
}
...
/// Detach stdio: chdir /, redirect stdin from /dev/null, stdout+stderr to the
/// log file. Must be called from the daemon process before it starts logging.
///
/// Assumes fds 0/1/2 are inherited from the CLI invocation, so the
/// freshly-opened files land at fd 3+. The handles drop after dup2 — only the
/// duplicated stdio descriptors remain.

...
use crate::model::{ScheduledTask, SchedulesFile};
use crate::paths;
...
/// Load tasks from a project's schedules.json. Returns an empty file if absent.
pub fn load_tasks(d_tag: &str) -> Result<SchedulesFile> {
...
pub fn find_project_for_task(task_id: &str) -> Result<Option<String>> {
    for d_tag in list_project_dtags()? {
        let file = load_tasks(&d_tag)?;
        if file.tasks.iter().any(|t| t.id == task_id) {

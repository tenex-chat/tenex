...
impl Lockfile {
    pub fn acquire(path: &Path) -> Result<Self> {
        ...
        if !try_flock(file.as_raw_fd())? {
            return Err(anyhow!("another tenex-summarizer holds {}", path.display()));
        }
    ...
    /// Return Some(pid) if a daemon currently holds the lock, None otherwise.
    /// Used by `status`. Does not retain the lock — the probe handle is dropped.
    pub fn probe(path: &Path) -> Result<Option<u32>> {
        ...
        if try_flock(probe.as_raw_fd())? {
            // We got the lock — nobody is running. Release it implicitly via Drop.
            return Ok(None);
        }

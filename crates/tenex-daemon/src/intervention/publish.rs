...
fn project_coordinate(inputs: &ReviewRequestInputs<'_>) -> String {
    format!(
        "31933:{}:{}",
        inputs.project_owner_pubkey, inputs.project_d_tag
    )
}
...
mod tests {
    ...
    fn sample_inputs<'a>(owner: &'a str, intervention_agent: &'a str) -> ReviewRequestInputs<'a> {
        ReviewRequestInputs {
            ...
            completing_agent_pubkey: "9999999999999999999999999999999999999999999999999999999999999999",
    ...
    fn event_is_kind_1_with_context_and_review_tags() {
        ...
        assert!(
            event
                .tags
                .contains(&vec!["p".to_string(), intervention_pubkey.to_string()])
        );
    ...
    fn manager_tag_omitted_when_same_as_intervention_agent() {
        ...
        let p_tags: Vec<&Vec<String>> = event.tags.iter().filter(|tag| tag[0] == "p").collect();

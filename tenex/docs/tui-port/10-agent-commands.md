...
| `category` | `AgentCategory?` | event `category` tag, normalized via `resolveCategory` (`agent-installer.ts:104–105`). |
| `inferredCategory` | `AgentCategory?` | written post-install by `categorizeAgent` if `category` is missing (`agent-installer.ts:243–266`). |

...
impl TenexConfigDoc {
    ...
    // ---- Telemetry -------------------------------------------------------
    //
    // Source: `TenexConfigSchema:81-94`. The TS code mutates the block via
    // spread shape, so the Rust setters here are granular (each preserves
    // every other field). The `analysis` sub-block has its own granular
    // accessors mirroring the TS resolved-defaults shape.

...
function flattenTree(span: TraceSpan, depth = 0, expandedSet: Set<string>, index = { current: 0 }): TreeNode[] {
...
function getSpanLabel(span: TraceSpan): { label: string; color: string; icon: string } {
  ...
  if (span.operationName.includes('ai.streamText') || span.operationName.includes('ai.generateText')) {
  ...
  return (
    <Box flexDirection="column">
      <Box borderStyle="single" borderColor="gray" paddingX={1}>
        <Text bold color="cyan">TENEX Trace Viewer</Text>
      ...
      <Box marginTop={1} borderStyle="single" borderColor="gray" paddingX={1}>
        <Text dimColor>
          Selected: <Text color="cyan">{selectedNode ? getSpanLabel(selectedNode.span).label : 'None'}</Text>
          {' | '}
          Duration: <Text color="yellow">{selectedNode ? (selectedNode.span.duration / 1000).toFixed(1) : '0'}ms</Text>
          {traceNavigation && (
            <>
              {' | '}
              Trace: <Text color="cyan">{traceNavigation.current}/{traceNavigation.total}</Text>
  ...
  return (
    <Box flexDirection="column">
      <Box borderStyle="double" borderColor="cyan" paddingX={1}>
        <Text bold color="cyan">Span Details</Text>
      ...
      <Box flexDirection="column" marginTop={1} paddingX={2}>
        ...
        {Object.keys(span.attributes).length > 0 && (
          <Box flexDirection="column" marginTop={1}>
            <Text bold underline>Attributes:</Text>
        ...
        {span.events.length > 0 && (
          <Box flexDirection="column" marginTop={1}>
            <Text bold underline>Events ({span.events.length}):</Text>
        ...
        {span.children.length > 0 && (
          <Box marginTop={1}>
            <Text bold underline>Children: {span.children.length} spans</Text>
          </Box>
        )}
      </Box>
    </Box>

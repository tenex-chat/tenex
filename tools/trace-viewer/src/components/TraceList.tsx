...
export function TraceList({ traces, loading, error, onSelect, onRefresh }: TraceListProps) {
  ...
  return (
    <Box flexDirection="column">
      <Box borderStyle="single" borderColor="gray" paddingX={1}>
        <Text bold color="cyan">TENEX Trace Viewer</Text>
        <Text dimColor> - Select a trace to view (↑↓ to navigate, Enter to select, r to refresh, q to quit)</Text>
      </Box>
      ...
      <Box marginTop={1} borderStyle="single" borderColor="gray" paddingX={1}>
        <Text dimColor>
          Showing {traces.length} recent traces | Selected: <Text color="cyan">{selectedIndex + 1}/{traces.length}</Text>
        </Text>
      </Box>
    </Box>
  );

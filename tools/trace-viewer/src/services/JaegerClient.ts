...
import type { TraceSpan, Trace } from '../types.js';
...
export class JaegerClient {
  ...
  constructor(baseUrl: string = 'http://localhost:16686') {
   ...
   */
  async getTraces(service: string = 'tenex-daemon', limit: number = 20): Promise<Array<{
    traceId: string;
    summary: string;
    duration: number;
    timestamp: number;
  ...
  private convertJaegerTrace(jaegerTrace: JaegerTrace): Trace {
    ...
    spans.forEach(span => {
      ...
      if (parentRef) {
        ...
        console.error(`[JaegerClient] Found child: ${span.operationName} (${span.spanID.substring(0, 8)}) -> parent: ${parentId.substring(0, 8)}`);
      }
    });
  ...
  private convertSpan(
    ...
    childrenMap: Map<string, JaegerSpan[]>
  ): TraceSpan {
